window.onload = function () {
  ...
  g.draw = function () {
    const bx = g.width / particleSize - 1;
    const by = g.height / particleSize - 1;
    ...
    const wdx = (window.screenX - wx) / particleSize;
    const wdy = (window.screenY - wy) / particleSize;
    wx = window.screenX;
    wy = window.screenY;
    ...
    for (i = 0, il = particles.length; i < il; i++) {
      var p = particles[i];
      const x1 = p.x * particleSize;
      const y1 = p.y * particleSize;
      const x2 = (p.x - p.gu) * particleSize;
      const y2 = (p.y - p.gv) * particleSize;
      ctx.moveTo(x1, y1);
    			ctx.lineTo(x2, y2);
      if (x1 < clearLeft) {
        clearLeft = x1;
